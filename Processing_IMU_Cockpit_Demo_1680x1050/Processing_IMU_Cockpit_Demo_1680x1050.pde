...
int width = 1680; // Width of GUI window on monitor in pixels
int height = 1050; // Height of GUI window on monitor in pixels
...
void draw() {
  ...
  pushMatrix(); // Built-in function that saves the current position of the coordinate system
    ...
    //Map
    pushMatrix();
      translate(385,-60);//425,11 (1600x900) - 588,66 (1280x800) - 506,11 (1440x900)
    ...
    // Artificial Horizon
    pushMatrix();
      translate(width/5.5, height/2.35); // (1600x900)(width/5.7, height/2.0); (1280x800)(width/4.15, height/1.8); (1440x900)(width/4.9, height/2.0);
      Horizon();
      pushMatrix();
...
void Borders() {
  ...
  fill(0);
  noStroke();
  rect(4*width/6, 0, width, 1.5*height);
