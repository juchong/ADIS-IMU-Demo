...
public class Processing_IMU_Cockpit_Demo_1920x1200 extends PApplet {
...
int height = 1200; // Height of GUI window on monitor in pixels
...
public void draw() {
  ...
  pushMatrix(); // Built-in function that saves the current position of the coordinate system
    ...
    //Map
    pushMatrix();
      translate(265,-140);//425,11 (1600x900) - 588,66 (1280x800) - 506,11 (1440x900)
    ...
    // Artificial Horizon
    pushMatrix();
      translate(width/6.5f, height/2.7f); // (1600x900)(width/5.7, height/2.0); (1280x800)(width/4.15, height/1.8); (1440x900)(width/4.9, height/2.0);
      Horizon();
      pushMatrix();
...
public void Borders() {
  ...
  fill(0);
  noStroke();
  rect(4*width/6, 0, width, 1.5f*height);
  rect(-4*width/6, 0, width, 1*height);
}
...
class LowPass {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Processing_IMU_Cockpit_Demo_1920x1200" };
    if (passedArgs != null) {
